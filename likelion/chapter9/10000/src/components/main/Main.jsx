    ...
    <main>
      <form className="cont-inp" onSubmit={handleSubmit}>
        <p className="txt-wannabe">
          나는
          <input type="text" placeholder="프로그래밍" onChange={(event) => setField(event.target.value)} /> 전문가가 될 것이다.
        </p>
        <p className="txt-time">
          그래서 앞으로 매일 하루에
          <input type="number" placeholder="5" onChange={(event) => setTime(event.target.value)} /> 시간씩 훈련할 것이다.
        </p>
        <button className="btn-exc" type="submit">
          나는 며칠동안 훈력을 해야 1만 시간이 될까?
        </button>
      </form>
      ...
      {day && (
        <section className="cont-result">
          ...
          <button type="button" className="btn-go" onClick={() => props.modalOpen(true)}>
            훈련하러 가기 GO!GO!
          </button>
